...
async function main() {
  ...
  // Update version
  const newVersion = runCommand(`npm version ${releaseType} --no-git-tag-version`, `Bumping ${releaseType} version`);
  ...
  // Commit version bump
  runCommand(`git add package.json package-lock.json`, 'Staging version files');
  ...
  console.log(`   npm install -g vibei18n@${newVersion.replace('v', '')}`);
